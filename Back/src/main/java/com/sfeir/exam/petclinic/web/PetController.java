...
public class PetController {
	...
	public String list(@RequestParam(value = "page", required = false) Integer page,
		...
		if (page != null || size != null) {
			...
			modelMap.addAttribute("pets",
					petDao.findPetEntries(page == null ? 0 : (page.intValue() - 1) * sizeNo, sizeNo));
	...
	public String delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page,
		...
		if (id == null)
			throw new IllegalArgumentException("An Identifier is required");
		petDao.remove(id);
		return "redirect:/pets?page=" + ((page == null) ? "1" : page.toString()) + "&size="
	...
	public String findPetsByNameAndWeight(@RequestParam("name") String name, @RequestParam("weight") Float weight,
			ModelMap modelMap) {
		if (name == null || name.length() == 0)
			throw new IllegalArgumentException("A Name is required.");
		if (weight == null)
			throw new IllegalArgumentException("A Weight is required.");
		modelMap.addAttribute("pets", petDao.findPetsByNameAndWeight(name, weight).getResultList());
		return "pets/list";
	}
	...
	public String findPetsByOwner(@RequestParam("owner") Owner owner, ModelMap modelMap) {
		if (owner == null)
			throw new IllegalArgumentException("A Owner is required.");
		modelMap.addAttribute("pets", petDao.findPetsByOwner(owner).getResultList());
		return "pets/list";
	}
	...
	public String findPetsBySendRemindersAndWeightLessThan(
		...
		if (weight == null)
			throw new IllegalArgumentException("A Weight is required.");
		modelMap.addAttribute("pets",
				petDao.findPetsBySendRemindersAndWeightLessThan(sendReminders, weight).getResultList());
		return "pets/list";
	}
	...
	public String findPetsByTypeAndNameLikeForm(ModelMap modelMap) {
		modelMap.addAttribute("pettypes", java.util.Arrays.asList(PetType.class.getEnumConstants()));
		return "pets/findPetsByTypeAndNameLike";
	}

	@RequestMapping(params = "find=ByTypeAndNameLike", method = RequestMethod.GET)
	public String findPetsByTypeAndNameLike(@RequestParam("type") PetType type, @RequestParam("name") String name,
			ModelMap modelMap) {
		if (type == null)
			throw new IllegalArgumentException("A Type is required.");
		if (name == null || name.length() == 0)
			throw new IllegalArgumentException("A Name is required.");
		modelMap.addAttribute("pets", petDao.findPetsByTypeAndNameLike(type, name).getResultList());
		return "pets/list";
	}
	...
	Converter<Owner, String> getOwnerConverter() {
		return new Converter<Owner, String>() {
			public String convert(Owner owner) {
				return new StringBuilder().append(owner.getFirstName()).append(" ").append(owner.getLastName())
						.toString();
			}
		};
	}
