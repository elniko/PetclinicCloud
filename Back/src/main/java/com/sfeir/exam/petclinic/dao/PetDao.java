...
public class PetDao {
	...
	public List<Pet> findPetEntries(int firstResult, int maxResults) {
		Query q = getEntityManager().createQuery("SELECT p FROM Pet p");
	...
	public Query findPetsByNameAndWeight(String name, Float weight) {
		Query q = getEntityManager()
				.createQuery("SELECT pet FROM Pet AS pet WHERE pet.name = :name AND pet.weight = :weight");
	...
	public Query findPetsBySendRemindersAndWeightLessThan(boolean sendReminders, Float weight) {
		Query q = getEntityManager().createQuery(
				"SELECT pet FROM Pet AS pet WHERE pet.sendReminders = :sendReminders AND pet.weight < :weight");
	...
	public Query findPetsByTypeAndNameLike(PetType type, String name) {
		Query q = getEntityManager().createQuery(
				"SELECT pet FROM Pet AS pet WHERE pet.type = :type AND pet.name like :name");
